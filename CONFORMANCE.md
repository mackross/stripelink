...
| CFG-02 | Auth/API base URLs resolve once at construction with precedence option > environment > documented default. SpendRequestBaseURL defaults to the resolved APIBaseURL and, when explicitly set, affects only spend-request calls. Later environment changes do not affect an existing client. | TS `config.ts:118-131`; `spend-request.ts` | `TestNewClientResolvesConfigurationOnce`; `TestSpendRequestsCreatePresenceAndRouting`; `TestPaymentMethodsListEndpointEnvelopeAndForwardCompatibility` | DONE |
...
| HTTP-03 | Caller cancellation and deadlines promptly abort both the HTTP attempt and any token wait. The returned chain matches the context cause, and shared credential work continues only while another live caller needs it. | Go safety | `TestCoreDoWrapsNetworkAndContextErrors`; `TestAuthPollCancellationPreservesPending`; `TestTokenProviderWaiterCancellationDoesNotCancelSharedRefresh`; `TestTokenProviderFinalCancellationPreventsLatePersistence` | DONE |
...
| AUTH-02 | A successful initiation persists a resumable PendingDeviceAuth containing the device code, interval, absolute expiry, verification URL, and phrase. If persistence fails, initiation returns that failure. | TS CLI storage flow; Go cohesive-session deviation | `TestAuthInitiatePersistsAbsoluteExpiry`; `TestAuthStorageFailuresNeverClaimSuccess` | DONE |
| AUTH-03 | PollDeviceAuthOnce sends exactly one unauthenticated device-code form POST. On success it returns all token fields, atomically persists them before success, and clears only the matching still-current pending authorization. | TS `auth.ts:137-155`; Go cohesive-session deviation | `TestAuthPollOnceClassifiesAndPersists`; `TestAuthPollTerminalClearsOnlyMatchingPending`; `TestAuthStalePollCannotCommitAfterNewFlowStarts` | DONE |
...
| AUTH-09 | RefreshToken and RevokeToken send one unauthenticated form POST to the documented endpoints, accept the 2xx range, and return structured auth errors otherwise. Direct calls do not mutate storage; Logout is the revoke-then-clear session operation and retains state on failure. | TS `auth.ts:189-240`; Go session policy | `TestAuthRefreshRevokeAndLogout`; `TestAuthLogoutFailureRetainsSession` | DONE |
...
| ID | Observable behavior | Source | Evidence | Status |
|---|---|---|---|---|
| STORE-01 | SetAuth computes ExpiresAt from ExpiresIn only when absent, preserves explicit ExpiresAt, rejects nil/invalid input without changing state, and does not mutate inputs. | TS `storage.ts:35-40`; Go safety | `TestAuthStorageValueSemanticsAndExpiry`; `TestAuthStorageRejectsInvalidInputWithoutMutation` | DONE |
| STORE-02 | Get/Set auth and pending values use defensive copies; `Update` commits a complete snapshot atomically and concurrent readers/writers are race-free. | Go safety deviation | `TestAuthStorageValueSemanticsAndExpiry`; `TestAuthStorageUpdateIsAtomicAndContextAware`; `TestMemoryStorageConcurrentAccess`; `TestFileStorageCoordinatesInstances` | DONE |
| STORE-03 | Expired pending authorization is atomically cleared and returned as nil; an unexpired value round-trips exactly. Auth expiry does not erase refresh credentials before refresh handling. | TS `storage.ts:107-115` | `TestAuthStorageValueSemanticsAndExpiry`; `TestTokenProviderFreshExpiredAndMissing` | DONE |
| STORE-04 | ClearAuth, ClearPendingDeviceAuth, and ClearAll remove exactly their documented state and are idempotent. MemoryStorage's zero value is usable. | TS `storage.ts`; Go zero-value convention | `TestAuthStorageClearOperations`; `TestMemoryStorageConcurrentAccess` | DONE |
...
| TOKEN-03 | Concurrent requests and Clients sharing one storage generation cause exactly one refresh call and all live waiters receive the result, for proactive and forced/401-style demand. | Go single-flight extension | `TestTokenProviderRefreshIsSingleFlightAndGenerationAware`; `TestTokenProviderRefreshCoalescesAcrossClientsSharingStorage` | DONE |
...
| TOKEN-08 | A canceled refresh waiter returns promptly without canceling work needed by another live caller. When no callers remain, shared work is canceled and cannot later mutate storage. | Go cancellation safety | `TestTokenProviderWaiterCancellationDoesNotCancelSharedRefresh`; `TestTokenProviderFinalCancellationPreventsLatePersistence` | DONE |
...
| SPEND-02 | Create sends authenticated JSON POST; required and representative nested monetary fields round-trip as `int64`. Approve routes to `/create_delegated` and is never serialized. | TS `spend-request.ts:168-190`; tests | `TestSpendRequestsCreatePresenceAndRouting`; `TestSpendRequestsCreateNestedValuesWithoutPrecisionLoss` | DONE |
| SPEND-03 | Optional request fields preserve omitted versus explicit false, zero, empty string, and empty array states. Encoding does not mutate caller params. | TS `JSON.stringify`; Go API-shape correction | `TestSpendRequestsCreatePresenceAndRouting`; `TestSpendRequestsUpdateEscapesIDAndPreservesPresence` | DONE |
...
| RES-07 | Same-host cache misses are single-flighted while different hosts proceed independently. Canceled waiters follow TOKEN-08 semantics and failed/malformed responses are not cached. | Go concurrency extension | `TestWebBotAuthSignURLSingleFlightSameHost`; `TestWebBotAuthSignURLDifferentHostsProceedConcurrently`; `TestWebBotAuthSignURLCanceledWaiterDoesNotCancelSharedWork`; `TestWebBotAuthSignURLCancelsWorkAfterLastWaiterLeaves`; `TestWebBotAuthSignURLAPITransportAndNoMutationReplay` | DONE |
...
| Concurrency | Token refresh and per-host signature misses are single-flight and generation-aware. | TypeScript does not coordinate concurrent calls. |
...
3. Optional scalars use pointers and optional slices preserve nil versus
   explicit empty. Local validation covers structural/safety invariants, not
   changeable server business rules or unknown enum values.
